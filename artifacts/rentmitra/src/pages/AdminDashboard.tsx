...
const TABS = ["Overview", "Memberships"] as const;
...
export default function AdminDashboard() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl pb-24">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-secondary p-1 rounded-xl w-fit mb-8">
        {TABS.map(tab => (
          <button
            key={tab}
          ...
          >
            {tab === "Memberships" && <CreditCard className="w-3.5 h-3.5 inline mr-1.5" />}
            {tab}
          </button>
        ))}
      </div>
